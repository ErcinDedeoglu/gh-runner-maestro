...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/var/log/gh-runner-maestro/maestro.log') if os.path.exists('/var/log/gh-runner-maestro') else logging.StreamHandler(sys.stdout)
    ]
)
...
class MaestroService:
    ...
    def _validate_configs(self):
        ...
        for entry in self.runner_configs:
            ...
            if not entry.get("token"):
                logger.warning(f"No token provided for {entry['url']}, using DEFAULT_RUNNER_TOKEN")
    ...
    def _initialize_managers(self):
        ...
        for entry in self.runner_configs:
            ...
            logger.info(f"Configured: {entry['count']} runners for {entry['url']} [labels={entry['labels']}]")
    ...
    def launch_runners(self):
        ...
        for entry in self.runner_managers:
            ...
            try:
                ...
                logger.info(f"[{url}] Currently running: {current_count} / Target: {desired_count}")
                ...
                for i in range(current_count, desired_count):
                    ...
                    try:
                        ...
                        logger.info(f"[{url}] Launched runner container: {container.name}")
                        launched += 1
    ...
    def monitor_runners(self):
        ...
        for entry in self.runner_managers:
            ...
            try:
                ...
                logger.info(f"[{url}] Status: {len(running)} running, {len(exited)} exited")
                ...
                for container in exited:
                    try:
                        ...
                        logger.warning(f"[{url}] Container {container.name} exited with code {exit_code}")
                        
                        # Remove crashed containers
                        container.remove(force=True)
                        logger.info(f"[{url}] Removed crashed container: {container.name}")
    ...
    def cleanup_runners(self):
        ...
        logger.info(f"Cleanup completed: {cleanup_success} successful, {cleanup_failed} failed")
